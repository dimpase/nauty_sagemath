/* dretodot.c  version 1.0; B D McKay & A Piperno, Feb 2013. */

#define USAGE "dretodot [-S#:#sixF#o#m#n#r#:#[r#]d#g] [infile [outfile.dot [outfile.dre]]]"

#define HELPTEXT \
" Read graphs and initial coloring in dreadnaut format.\n\
 Write graphs in dot format to outfile.dot.\n\
 Write graphs and partitions in dreadnaut format to outfile.dre.\n\
 The name \"outfile.dre\" may eventually coincide with infile.\n\
\n\
   -v    Set verbose mode (default NO).\n\
   -S#:# Set maximum width and height of the drawing, in inches\n\
         (default 10 x 6.18).\n\
   -s    Scale the size of the drawing according to the number\n\
         of vertices of the graph (default NO).\n\
   ...
   -n#   Scale the size of vertices in the drawing.\n\
...
{
	int m, n, c, a, i, j, k, end;
    size_t j1;
	int argnum, initorg, cell, numCol, modcode, vtx, refcode;
	char *arg, sw;
	boolean badargs, make_ranks;
	boolean rswitch, dswitch, mswitch, sswitch, nswitch, oswitch, iswitch, xswitch, Sswitch, gswitch, fswitch, vswitch, rswitch1;
	...
	rswitch = dswitch = mswitch = sswitch = nswitch = oswitch = iswitch = xswitch = Sswitch = gswitch = fswitch = vswitch = rswitch1 = FALSE;
    ...
    //if (numcells > 4096) gt_abort(">E dretodot: too many colors (max: 4096)\n");
    ...
    if (xswitch) {
        ...
        //traces_opts.writeautoms = TRUE;  // THIS MUST BE CHANGED
