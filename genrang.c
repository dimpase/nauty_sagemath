/* genrang.c  version 2.0; B D McKay, Jun 27, 2015 */
/* TODO:  Check allocs for no edges */

#define USAGE \
"genrang [-P#|-P#/#|-e#|-r#|-R#|-d#] [-l#] [-m#] [-t] [-a] \n" \
"         [-s|-g|-z] [-S#] [-q] n num [outfile]"
...
main(int argc, char *argv[])
{
    ...
    if ((P1switch!=0) + (eswitch!=0) + (rswitch!=0) + (dswitch!=0)
        ...
        gt_abort(">E genrang: -PerRdt are incompatible\n");
    ...
    if (digraph && (Rswitch || multmax>1 || loopmax>1 || dswitch || tswitch))
        gt_abort(">E genrang: -z is only compatible with -P, -e and -l1\n");
    ...
    for (nout = 1; nout <= numgraphs; ++nout)
    {
        if (eswitch)
        {
	    if (digraph) ranarcs(evalue,loopmax>0,g,m,n);
	    else         ranedges(evalue,loopmax>0,g,m,n);
	}
	else if (dswitch) dmodel_sg(rvalue,&sg,n);
        ...
        else grandgraph(g,digraph,loopmax>0,P1value,P2value,m,n);
