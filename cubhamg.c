/* cubhamg.c : pick those inputs that are nonhamiltonian and
 ...
 Usage:
cubhamg [-#] [-v|-V] [-a#-#|-i|-I|-o|-x|-e|-E] [-b|-t] [infile [outfile]]
	...
	-a#-#  If the two numbers are v and i, then the i-th edge
	    out of vertex v is required to be not in the cycle.
	    It must be that i=1..3 and v=0..n-1.
            You can use any number of -a switches to forbid multiple 
	...
	-b  Require biconnectivity
        ...
        -a, -#, -R and -T are ignored for -i, -I, -x, -o, -e, -E

	B. D. McKay, Nov 1995 + Aug 1996 + Feb 2002 + Jul 2008
...
#define MAXA  100   /* max number of -a switches */
...
static int
isham(cubgraph cub,
      int n, int ne, int weight, int *vv, int *vi, int nvv, int *cyc)
/* test if hamiltonian; optionally return a cycle 
   Forbid the vi[i]-th nbr of vv[i], for i=0..nvv-1 
     WARNING: vi[i] is numbered starting at 1  */
{
        int i,j,k;
        int nmax,ch;
        cubgraph cubcopy;
   ...
   number of edges not present in any cycle */
{
	...
	ch = isham(cub,n,ne,weight,vv,vi,0,cyc);
	...
	for (jj = 0; jj < 3; ++jj)
	{
		...
		vv[0] = ii;
		vi[0] = jj+1;
		ch = isham(cub,n,ne,weight,vv,vi,1,cyc);
                if (ch == HABORT) return -2;
		if (ch == NO)
		{
		    x0[nbad] = ii;
...
main(int argc, char *argv[])
{
        ...
        boolean badargs,biconn,triconn;
	...
	sparsegraph sg;
	int vv[MAXA],vi[MAXA],nvv,verbose,cyc[MAXN];
	double t0,t1;
	cubgraph cub;
        char *arg;
	int codetype;
	...
	repeats = 1;
     ...
     /* parse argument list */
		...
		else if (arg[1] == 'a')
		{
		    if (nvv == MAXA)
		    {
			fprintf(stderr,">E cubhamg : too many -a switches\n");
			exit(1);
		    }
		    ...
		    {
			fprintf(stderr,
                               ">E cubhamg : second arg of -a must be 1..3\n");
			exit(1);
		    }
		    ++nvv;
		}
	...
	{
	    fprintf(stderr,
         ">E Usage: cubhamg [-#] [-v | -V] [-a#-#] [infile [outfile]]\n");
	    exit(1);
	}
...
//else                  outproc = writeg6;
	...
	while (read_sg(infile,&sg))
	{
	    ...
	    else if ((ch = isham(cub,n,ne,weight,vv,vi,nvv,cyc)) == NO)
	    {
	   	if (verbose)
                    fprintf(msgfile,"Input " COUNTER_FMT
                                    " is not hamiltonian.\n",numread);
